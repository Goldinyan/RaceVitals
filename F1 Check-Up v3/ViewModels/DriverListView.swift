    ...
    var filteredTeams: [Team] {
        ...
        if let year = selectedYear {
            return filteredBySearch.filter { team in
                (team.currentDrivers(language: languagekey)
                    + team.oldDriver(language: languagekey)).contains {
                        driver in
                        driver.seasons.contains {
                            $0.year == year && $0.team == team.name
                        }
                    }
            ...
            ZStack {
                ...
                HStack {
                    Button(action: {
                        ...
                        Image(
                            systemName: ShowOptions
                                ? "pencil"
                                : (SearchBar ? "xmark" : "magnifyingglass")
                        )
                        .font(.title2)
                    }
                }
...
struct HeaderView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
                                    ...
                                    VStack {
                                        HStack {
                                            ...
                                            if !Options[i].details.isEmpty {
                                                Button {
                                                    if toggleFavView {
                                                        if AppState.FavIndicies
                                                            .contains(i)
                                                        {
                                                            AppState.FavIndicies
                                                                .remove(i)
                                                        } else {
                                                            AppState.FavIndicies
                                                                .insert(i)
            ...
            .frame(
                minHeight: ShowOptions
                    ? UIScreen.main.bounds.height * 0.8
                    : UIScreen.main.bounds.height * 0.06
            )
            ...
            VStack {
                ...
                ZStack(alignment: .bottom) {
                    ...
                    Rectangle()
                        ...
                        .offset(
                            x: (AppState.SelectedOption == .Favorites)
                                ? -geometry.size.width * 0.3
                                : (AppState.SelectedOption == .Search)
                                    ? 0.3
                                    : (AppState.SelectedOption == .History)
                                        ? geometry.size.width * 0.3 : 0
                        )
                ...
                VStack {
                    if AppState.SelectedOption == .Favorites {
                        ForEach(
                            AppState.ShowFavorite(topic: AppState.SelectedView),
                            id: \.self
                        ) { i in
                            HStack {
                                Spacer()
                        ...
                        ForEach(
                            AppState.ShowHistory(topic: AppState.SelectedView),
                            id: \.self
                        ) { i in
                            HStack {
                                Spacer()
